...
class TestPowerFlow(unittest.TestCase):
    ...
    def test_ACPF_nr_controls_support(self):
        ...
        method.set_parameters(
            {"quiet": True, "solver": "nr", "Q_mode": "regulating", "Q_limits": False}
        )
        method.solve(net)
        self.assertEqual(method.get_results()["solver status"], "solved")
        ...
        method = gopt.power_flow.ACPF()
        method.set_parameters(
            {"quiet": True, "solver": "nr", "Q_mode": "free", "Q_limits": False}
        )
        self.assertRaises(ValueError, method.solve, net)

        method = gopt.power_flow.ACPF()
        method.set_parameters(
            {"quiet": True, "solver": "nr", "Q_mode": "free", "Q_limits": True}
        )
        self.assertRaises(ValueError, method.solve, net)

        method = gopt.power_flow.ACPF()
        method.set_parameters(
            {"quiet": True, "solver": "nr", "Q_mode": "locked", "Q_limits": True}
        )
        self.assertRaises(ValueError, method.solve, net)

        method = gopt.power_flow.ACPF()
        method.set_parameters(
            {"quiet": True, "solver": "nr", "Q_mode": "locked", "Q_limits": False}
        )
        self.assertRaises(ValueError, method.solve, net)
        ...
        method = gopt.power_flow.ACPF()
        method.set_parameters(
            {"quiet": True, "solver": "nr", "shunt_mode": "free", "shunt_limits": False}
        )
        self.assertRaises(ValueError, method.solve, net)

        method = gopt.power_flow.ACPF()
        method.set_parameters(
            {"quiet": True, "solver": "nr", "shunt_mode": "free", "shunt_limits": True}
        )
        self.assertRaises(ValueError, method.solve, net)
        ...
        method = gopt.power_flow.ACPF()
        method.set_parameters(
            {"quiet": True, "solver": "nr", "tap_mode": "locked", "tap_limits": False}
        )
        method.solve(net)
        self.assertEqual(method.get_results()["solver status"], "solved")
        ...
        method = gopt.power_flow.ACPF()
        method.set_parameters(
            {"quiet": True, "solver": "nr", "tap_mode": "free", "tap_limits": False}
        )
        self.assertRaises(ValueError, method.solve, net)

        method = gopt.power_flow.ACPF()
        method.set_parameters(
            {"quiet": True, "solver": "nr", "tap_mode": "free", "tap_limits": True}
        )
        self.assertRaises(ValueError, method.solve, net)

        # Impedance correction
        method = gopt.power_flow.ACPF()
        method.set_parameters({"quiet": True, "solver": "nr", "y_correction": False})
        method.solve(net)
        self.assertEqual(method.get_results()["solver status"], "solved")
    ...
    def test_ACPF_keep_all(self):
        for case in utils.test_cases:
            if os.path.splitext(case)[1] == ".raw":
                ...
                if (
                    net.num_buses == net_oos.num_buses
                    and net.num_generators == net_oos.num_generators
                    and net.num_loads == net_oos.num_loads
                    and net.num_shunts == net_oos.num_shunts
                    and net.num_branches == net_oos.num_branches
                ):
                    continue

                method = gopt.power_flow.ACPF()
                method.set_parameters(
                    {"solver": "augl", "quiet": True, "v_mag_warm_ref": True}
                )
                ...
                self.assertEqual(
                    r["network snapshot"].bus_v_max, r_oos["network snapshot"].bus_v_max
                )
                self.assertEqual(
                    r["network snapshot"].bus_v_min, r_oos["network snapshot"].bus_v_min
                )
                self.assertEqual(
                    r["network snapshot"].bus_P_mis, r_oos["network snapshot"].bus_P_mis
                )
                self.assertEqual(
                    r["network snapshot"].bus_Q_mis, r_oos["network snapshot"].bus_Q_mis
                )

                self.assertRaises(
                    AssertionError,
                    pf.tests.utils.compare_networks,
                    self,
                    r["network snapshot"],
                    r_oos["network snapshot"],
                )
    ...
    def test_ACPF_parameters(self):
        ...
        self.assertRaises(
            gopt.power_flow.method_error.PFmethodError_BadParams,
            acpf.set_parameters,
            {"foo": "bar"},
        )
        ...
        for sol in solvers:
            ...
            self.assertRaises(
                gopt.power_flow.method_error.PFmethodError_SolverError,
                acpf.solve,
                net,
                {"quiet": True},
            )
    ...
    def test_ACOPF_parameters(self):
        ...
        self.assertEqual(
            params["solver_parameters"]["ipopt"]["hessian_approximation"], "test"
        )
        ...
        self.assertNotEqual(
            new_params["solver_parameters"]["augl"]["beta_small"],
            params["solver_parameters"]["augl"]["beta_small"],
        )
        self.assertNotEqual(
            new_params["solver_parameters"]["inlp"]["maxiter"],
            params["solver_parameters"]["inlp"]["maxiter"],
        )
        self.assertNotEqual(
            new_params["solver_parameters"]["ipopt"]["hessian_approximation"],
    ...
    def test_ACPF_with_gen_outages(self):
        ...
        for solver in ["nr", "augl"]:
            method.set_parameters(params={"solver": solver, "quiet": True})
            method.solve(net)
            ...
            for bus in net.buses:
                if bus.is_v_set_regulated():
                    ...
                    self.assertEqual(
                        net.get_generator(gen.index).Q, net2.get_generator(gen.index).Q
                    )
                    self.assertNotEqual(
                        net.get_generator(gen.index).Q, net1.get_generator(gen.index).Q
                    )
                    gen.in_service = True
    ...
    def test_ACPF_with_branch_outages(self):
        ...
        for solver in ["nr", "augl"]:
            method.set_parameters(
                params={"solver": solver, "tap_mode": "regulating", "quiet": True}
            )
            method.solve(net)
            ...
            for bus in net.buses:
                if bus.is_regulated_by_tran() and bus.number == 1:
                    ...
                    net2 = method.get_results()["network snapshot"]
                    self.assertFalse(
                        net.get_branch(tran.index).has_flags("variable", "tap ratio")
                    )
                    self.assertTrue(
                        net1.get_branch(tran.index).has_flags("variable", "tap ratio")
                    )
                    self.assertFalse(
                        net2.get_branch(tran.index).has_flags("variable", "tap ratio")
                    )
    ...
    def test_ACPF_nr_bus_type_heuristics(self):
        ...
        for case in utils.test_cases:
            ...
            method = gopt.power_flow.new_method("ACPF")
            method.set_parameters(params={"solver": "nr", "quiet": True})
            method.solve(net)
            ...
            for bus in net_snap.buses:
                if bus.is_regulated_by_gen() and not bus.is_slack():
                    ...
                    if np.abs(bus.v_mag - bus.v_set) < eps:  # v at set
                        ...
                        for gen in bus.reg_generators:
                            if (
                                np.abs(gen.Q - gen.Q_max) > eps
                                and np.abs(gen.Q - gen.Q_min) > eps
                            ):
                                Qtotal += gen.Q
                                norm += gen.Q_par
                        for gen in bus.reg_generators:
                            if (
                                ...
                                self.assertLess(
                                    np.abs(gen.Q - gen.Q_par * Qtotal / norm), eps
                                )
                    else:  # v not at set
                        num = 0
                        for gen in bus.reg_generators:
    ...
    def test_ACPF_solutions(self):
        ...
        skipcases = ["ieee59_convert_to_zil_to_solve.raw", "ieee25_zcor.raw"]
                ...
                for solver in ["nr", "augl", "ipopt", "inlp"]:
                    ...
                    if solved:
                        ...
                        self.assertLessEqual(
                            results["network snapshot"].bus_P_mis, bus_P_mis
                        )
                        method.update_network(net)
                        ...
                        for bus in net_snap.buses:
                            ...
                            self.assertAlmostEqual(
                                bus.P_mismatch,
                                net.get_bus_from_number(bus.number).P_mismatch,
                                places=10,
                            )
                        ...
                        self.assertAlmostEqual(
                            results["network snapshot"].bus_P_mis,
                            net.bus_P_mis,
                            places=10,
                        )
                        self.assertAlmostEqual(
                            results["network snapshot"].bus_Q_mis,
                            net.bus_Q_mis,
                            places=10,
                        )
                        ...
                        self.assertLess(
                            np.linalg.norm(
                                resultsMP["network snapshot"].bus_P_mis
                                - netMP.bus_P_mis,
                                np.inf,
                            ),
                            1e-10,
                        ...
                        self.assertLess(
                            np.linalg.norm(
                                resultsMP["network snapshot"].bus_Q_mis
                                - netMP.bus_Q_mis,
                                np.inf,
                            ),
                            1e-10,
                        ...
                        self.assertLess(
                            np.linalg.norm(
                                resultsMP["network snapshot"].gen_P_cost
                                - netMP.gen_P_cost,
                                np.inf,
                            ),
                            1e-10,
                        ...
                        if sol_data is not None:
                            ...
                            for bus_num, val in list(bus_data.items()):
                                ...
                                for t in range(T):
                                    ...
                                    v_ang_error.append(
                                        np.abs(busMP.v_ang[t] * 180.0 / np.pi - v_ang)
                                    )
                                v_mag_error.append(np.abs(bus.v_mag - v_mag))
                                v_ang_error.append(
                                    np.abs(bus.v_ang * 180.0 / np.pi - v_ang)
                                )
    ...
    def test_ACOPF_solutions(self):
        ...
        for case in utils.test_cases:
            ...
            try:
                ...
                self.assertNotEqual(
                    method_ipopt.results["network snapshot"].gen_P_cost, gen_P_cost
                )
            ...
            self.assertEqual(net.gen_P_cost, gen_P_cost)
            self.assertNotEqual(
                method_inlp.results["network snapshot"].gen_P_cost, gen_P_cost
            )
            ...
            self.assertEqual(net.gen_P_cost, gen_P_cost)
            self.assertNotEqual(
                method_augl.results["network snapshot"].gen_P_cost, gen_P_cost
            )
    ...
    def test_DCOPF_solutions(self):
        ...
        for case in utils.test_cases:
            ...
            method.set_parameters({"quiet": True, "tol": 1e-6, "thermal_limits": True})
            ...
            try:
                ...
                self.assertTrue(
                    np.all(method.results["network snapshot"].gen_P_cost != gen_P_cost)
                )
            except gopt.power_flow.PFmethodError:
            ...
            self.assertLess(
                np.linalg.norm(results["network snapshot"].bus_P_mis - net.bus_P_mis, np.inf),
                1e-10,
            )
            self.assertLess(
                np.linalg.norm(results["network snapshot"].bus_Q_mis - net.bus_Q_mis, np.inf),
                1e-10,
            )
            self.assertLess(
                np.linalg.norm(results["network snapshot"].gen_P_cost - net.gen_P_cost, np.inf),
                1e-10,
            )
            ...
            for t in range(T):
                for bus in net.buses:
                    if not bus.is_slack():
                        ...
                        self.assertEqual(
                            bus.sens_v_ang_u_bound[t], mu0[bus.index_v_ang[t]]
                        )
                        self.assertEqual(
                            bus.sens_v_ang_l_bound[t], pi0[bus.index_v_ang[t]]
                        )
                    for branch in bus.branches_k:
                        self.assertEqual(
                            branch.sens_P_u_bound[t], mu0[net.num_vars + row]
                        )
                        self.assertEqual(
                            branch.sens_P_l_bound[t], pi0[net.num_vars + row]
                        )
                        row += 1
            ...
            for load in net.loads:
                load.P_max = load.P + 1.0
                load.P_min = load.P - 1.0
            ...
            self.assertEqual(method.results["solver status"], "solved")
            self.assertTrue(
                np.all(net.gen_P_cost - net.load_P_util < gen_P_cost1 - load_P_util1)
            )
            ...
            for t in range(T):
                for bus in net.buses:
                    if not bus.is_slack():
                        self.assertEqual(bus.v_ang[t], xx[bus.index_v_ang[t]])
                        self.assertEqual(
                            bus.sens_v_ang_u_bound[t], mu2[bus.index_v_ang[t]]
                        )
                        self.assertEqual(
                            bus.sens_v_ang_l_bound[t], pi2[bus.index_v_ang[t]]
                        )
                for gen in net.generators:
                    self.assertEqual(gen.P[t], xx[gen.index_P[t]])
    ...
    def test_ACPF_initialize_problem(self):
        ...
        self.assertLess(
            np.linalg.norm(
                result_steps["network snapshot"].get_var_values() - problem.x
            ),
            1e-6,
        )
        self.assertLess(
            np.linalg.norm(
                result_steps["network snapshot"].get_var_values()
                - unet.get_var_values()
            ),
            1e-6,
        )
        self.assertLess(
            np.linalg.norm(
                result_steps["network snapshot"].get_var_values()
                - result_steps["solver primal variables"]
            ),
            1e-6,
        )
        ...
        self.assertLess(
            np.linalg.norm(
                result_orig["network snapshot"].get_var_values()
                - result_orig["solver primal variables"]
            ),
            1e-6,
        )
        self.assertLess(
    ...
    def test_ACPF_with_inter_area_transfer(self):
        ...
        raise unittest.SkipTest(
            "inter-area transfer disabled in RAW Parser, skipping test"
        )
    ...
    def run_test_ACPF_with_inter_area_transfer(self, method_name="opt"):
        ...
        if not os.path.isfile(case):
            raise unittest.SkipTest(
                "file not available, skip testing inter-area transfer in {}".format(
                    method_name.upper()
                )
            )
            ...
            if num_gens > 1:
                for i in range(1, num_gens):
                    ...
                    self.assertGreaterEqual(
                        bus.generators[i].P, bus.generators[i].P_min
                    )
                    self.assertAlmostEqual(bus.generators[i].P, bus.generators[i].P, 8)

    def test_acpf_with_zip_loads(self):
        case = os.path.join(
            "tests", "resources", "cases", "ieee59_convert_to_zil_to_solve.raw"
        )
        if not os.path.isfile(case):
            raise unittest.SkipTest("file not available")
        ...
        acpf = gopt.power_flow.ACPF()
        acpf.set_parameters(
            params={"solver": "nr", "loads_2_ZIP": False, "quiet": True}
        )
        try:
            acpf.solve(net)
        ...
        self.assertAlmostEqual(
            load_zip.P, load_zip.comp_cp + load_zip.comp_ci + load_zip.comp_cg, 4
        )
        self.assertAlmostEqual(
            load_zip.Q, -(load_zip.comp_cb - load_zip.comp_cq - load_zip.comp_cj), 4
        )
        ...
        with open(psse_res_file, "r") as f:
            ...
            for row in reader:
                ...
                self.assertTrue(np.abs(bus.v_ang * 180.0 / np.pi - float(row[3])) < 0.2)
    ...
    def run_test_ACPF_with_phase_shifter_P_mode(self, method_name="nr"):
        ...
        if not os.path.isfile(case):
            raise unittest.SkipTest(
                "file not available, skip testing {} with phase shifter Pmode".format(
                    method_name.upper()
                )
            )
        ...
        for tr in shifters:
            self.assertTrue(tr.phase > tr.phase_min and tr.phase < tr.phase_max)
        ...
        for tr in shifters:
            self.assertTrue(tr.phase >= tr.phase_min and tr.phase <= tr.phase_max)
            self.assertTrue(tr.P_km >= tr.P_min and tr.P_km <= tr.P_max)
    ...
    def run_test_ACPF_with_tap_changer_Q_mode(self, method_name="nr"):
        ...
        if not os.path.isfile(case1):
            raise unittest.SkipTest(
                "file not available, skip testing tap changer Qmode in {}".format(
                    method_name.upper()
                )
            )
        if not os.path.isfile(case2):
            raise unittest.SkipTest(
                "file not available, skip testing tap changer Qmode out bounds in {}".format(
                    method_name.upper()
                )
            )
        if not os.path.isfile(case3):
            raise unittest.SkipTest(
                "file not available, skip testing tap changer Qmode out bounds with violations in {}".format(
                    method_name.upper()
                )
            )
        ...
        for tr in taps:
            self.assertTrue(tr.ratio > tr.ratio_min and tr.ratio < tr.ratio_max)
            self.assertTrue(tr.Q_km >= tr.Q_min and tr.Q_km <= tr.Q_max)
    ...
    def test_ACPF_with_wind_machines(self):
        ...
        wind_gens = [g for g in net.generators if g.is_nonconventional_machine()]
        ...
        for gen in net_nr.generators:
            if gen.is_nonconventional_machine():
                if gen.is_machine_with_fixed_power_factor():
                    gen_opt = net_opt.get_generator_from_name_and_bus_number(
                        gen.name, gen.bus.number
                    )
    ...
    def test_ACPF_SWSH_HEUR(self):
        ...
        method.set_parameters(
            {"quiet": True, "shunt_mode": "regulating", "feastol": 0.001}
        )
        method.solve(net)
        method.update_network(net)
